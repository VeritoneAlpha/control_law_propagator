...
class Blackboard:
    ...
    def __init__(self):
        ...
        self.q_p_u_dict = {'q_mf':{}, 'p_l':{}, 'p_mf':{}, 'u_mf':{}, 'q_mf_dot':{}}
        self.bar_dict = {}
        self.agents=[]
    ...
    def update_q_p_u_dict(self, agent):
        ...
        for state_ix in agent.state_indices:
            ...
            self.q_p_u_dict['q_mf_dot'][str(state_ix)] = agent.q_s_dot[state_ix-1]
            
        for control_ix in agent.control_indices:
            self.q_p_u_dict['u_mf'][str(control_ix)] = agent.qpu_vec[3*agent.state_dim:][control_ix-1]
...
def construct_mf_vectors(sliding_window_instance):
    '''
    Helper function to get q_mf, q_mf_dot, u_mf. 
    Inputs:
    ...
    u_mf_shape = len(bb.q_p_u_dict['u_mf'].items())
    ...
    for u_ix, u_val in bb.q_p_u_dict['u_mf'].items():
        # if this index does NOT PERTAIN to this agent, then fill in q_mf with value from the blackboard
        # if the index does PERTAIN to this agent, then fill in q_mf with the value from qpu_vec
