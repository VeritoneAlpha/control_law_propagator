...
def Betas(self, q_mf, p_mf, u_mf, u_s, q_s_dot, q_mf_dot, p_mf_dot, q_s, p_l):
    ...
    for j in range(self.control_dim):
        ...
        for k in range(len(self.control_indices)):
            ...
            Beta_mf_k = H_mf_u[j]*(np.dot(self.q_rhs_H_mf_u(q_mf, p_mf)[k], q_s_dot) + np.dot(self.p_rhs_H_mf_u(q_mf, p_mf)[k], p_mf_dot)) + \
                        H_mf_u[k]*(np.dot(self.q_rhs_H_mf_u(q_mf, p_mf)[j], q_s_dot) + np.dot(self.p_rhs_H_mf_u(q_mf, p_mf)[j], p_mf_dot))
...
def alphas(self, q_mf, p_mf, u_mf, u_s, q_s_dot, q_mf_dot, p_mf_dot, q_s, p_l, H_l_D, p_l_dot):
    ...
    for j in range(self.control_dim):
        ...
        H_mf_u = self.H_MF_u(q_mf, p_mf)
        H_l_u = self.H_l_u(q_s, p_l)
        H_mf_nou = self.H_MF_nou(q_mf, p_mf)
        H_l_nou = self.H_l_nou(q_mf, p_mf, lambda_l)
        import pdb; pdb.set_trace()
        alpha_mf_j = H_mf_u[j]*(np.dot(self.q_rhs_H_mf_nou(q_mf, p_mf), q_s_dot) + np.dot(self.p_rhs_H_mf_nou(q_mf, p_mf), p_mf_dot)) +\
                        (H_mf_nou-H_l_D)*(np.dot(self.q_rhs_H_mf_u(q_mf, p_mf)[j], q_s_dot) + np.dot(self.p_rhs_H_mf_u(q_mf, p_mf)[j], p_mf_dot))
