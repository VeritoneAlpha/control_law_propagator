...
class Agent1:
    ...
    def p_rhs_H_l_u(self, q_s, p_l):
        p_rhs_H_l_u = np.array([p_l])
        assert np.shape(p_rhs_H_l_u)==(self.control_dim, self.state_dim), 'p_rhs_H_l_u should return something with same dimensions as Jacobian'
        return p_rhs_H_l_u
    
    def q_rhs_H_l_u(self, q_s, p_l):
        q_rhs_H_l_u = np.array([p_l])
        assert np.shape(q_rhs_H_l_u)==(self.control_dim, self.state_dim), 'q_rhs_H_l_u should return something with same dimensions as Jacobian'
        return q_rhs_H_l_u
